...
export const ColorSchemeDialog: React.FC<ColorSchemeDialogProps> = ({ setDesignScheme, designScheme }) => {
  ...
  return (
    <div>
      <IconButton title="設計調色盤" className="transition-all" onClick={() => setIsDialogOpen(!isDialogOpen)}>
      ...
      <DialogRoot open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <Dialog>
          <div className="py-4 px-4 min-w-[480px] max-w-[90vw] max-h-[85vh] flex flex-col gap-6 overflow-hidden">
            <div className="">
              <DialogTitle className="text-2xl font-bold text-bolt-elements-textPrimary">設計調色盤與特性</DialogTitle>
              <DialogDescription className="text-bolt-elements-textSecondary leading-relaxed">
                自訂您的調色盤、字體和設計特性。這些偏好設定將引導 AI 創建符合您風格的設計。
              </DialogDescription>
            </div>
            ...
            <div className="flex gap-1 p-1 bg-bolt-elements-bg-depth-3 rounded-xl">
              {[
                { key: 'colors', label: '顏色', icon: 'i-ph:palette' },
                { key: 'typography', label: '字體', icon: 'i-ph:text-aa' },
                { key: 'features', label: '特性', icon: 'i-ph:magic-wand' },
              ].map((tab) => (
                <button
                  key={tab.key}
            ...
            <div className="flex justify-between items-center">
              <div className="text-sm text-bolt-elements-textSecondary">
                {Object.keys(palette).length} 個顏色 • {font.length} 種字體 • {features.length} 個特性
              </div>
